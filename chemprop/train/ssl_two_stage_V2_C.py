...
def build_polymer_graph(smiles):
    ...
    # Calculate ensemble molecular weight
    total_weight = 0.0
...
class NodeEdgeMaskingTask:
    ...
    """
    def __init__(self, model, mask_atoms, mask_edges):
        self.model = model
    ...
    def train_step(self, batch, optimizer, device):
        ...
        for g_idx in range(batch_size):
            ...
            if node_indices.numel() > 0:
                num_mask = min(self.mask_atoms, node_indices.numel())
                perm = torch.randperm(node_indices.numel())[:num_mask]
                sel_nodes = node_indices[perm]
                mask_atom_indices.extend(sel_nodes.tolist())
            
            if edge_indices.numel() > 0:
                num_mask = min(self.mask_edges, edge_indices.numel() // 2)  # Divide by 2 to account for bidirectional edges
    ...
    def eval_step(self, batch, device):
        ...
        for g_idx in range(batch_size):
            ...
            if edge_indices.numel() > 0:
                num_mask = min(self.mask_edges, edge_indices.numel() // 2)
                perm_e = torch.randperm(edge_indices.numel())[:num_mask]
                sel_edges = edge_indices[perm_e]
                for ei in sel_edges:
                    ei = int(ei.item())
...
class GraphLevelTask:
    ...
    def train_step(self, batch, optimizer, device):
        ...
        # Compute loss
        loss = F.mse_loss(pred_graph, mol_weights)
    ...
    def eval_step(self, batch, device):
        ...
        loss = F.mse_loss(pred_graph, mol_weights).item()
...
def run_pretraining_epoch(task, loader, device, optimizer=None, train=True):
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', patience=args.scheduler_patience, factor=0.5
    )
    
    node_edge_task = NodeEdgeMaskingTask(model, args.mask_atoms, args.mask_edges)
    ...
    for epoch in range(1, args.epochs_node_edge + 1):
        ...
        if epoch == 1:
            ...
            with open(loss_csv_path, 'w') as f:
                f.write('epoch,train_loss,val_loss,train_loss_node,train_loss_edge,val_loss_node,val_loss_edge\n')
        ...
        with open(loss_csv_path, 'a') as f:
            f.write(f"{epoch},{train_loss:.6f},{val_loss:.6f},{train_task_losses.get('loss_node', 0):.6f},"
                    ...
                    f"{val_task_losses.get('loss_edge', 0):.6f}\n")
    ...
    # Initialize new optimizer for stage 2
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='min', patience=args.scheduler_patience, factor=0.5
    )
    ...
    for epoch in range(1, args.epochs_graph + 1):
        ...
        if epoch == 1:
            # Create new file with header
            with open(loss_csv_path, 'w') as f:
                f.write('epoch,train_loss,val_loss,train_loss_graph,val_loss_graph\n')
        
        # Append current epoch losses
        with open(loss_csv_path, 'a') as f:
            f.write(f"{epoch},{train_loss:.6f},{val_loss:.6f},"
                    f"{train_task_losses.get('loss_graph', 0):.6f},{val_task_losses.get('loss_graph', 0):.6f}\n")
    ...
    # Also save a copy as model.pt for compatibility
